    ...
    public void updateClinicalCodeTable(String filePath, String validReadCodesFile, Date dataDate) throws Exception {
        ...
        try {
            ...
            sql = "INSERT IGNORE INTO emis_clinical_code (code_id, code_type, read_term, read_code, snomed_concept_id,"
                    ...
                    + " SELECT s.CodeId,"
                    + " IF(s.EmisCodeCategoryDescription != '', EmisCodeCategoryDescription, null),"
                    + " IF(s.Term != '', s.Term, null),"
                    + " IF(s.ReadTermId != '', s.ReadTermId, null),"
                    + " IF(s.SnomedCTConceptId != '', s.SnomedCTConceptId, null),"
                    + " IF(s.SnomedCTDescriptionId != '', s.SnomedCTDescriptionId, null),"
                    + " IF(x.SnomedTerm != '', x.SnomedTerm, null),"
                    + " IF(s.NationalCode != '', s.NationalCode, null),"
                    + " IF(s.NationalCodeCategory != '', s.NationalCodeCategory, null),"
                    + " IF(s.NationalDescription != '', s.NationalDescription, null),"
                    + " IF(s.ParentCodeId != '', s.ParentCodeId, null),"
                    + " IF(x.AdjustedCode != '', x.AdjustedCode, null),"
                    + " x.IsEmisCode,"
            ...
            sql = "UPDATE emis_clinical_code t"
                    ...
                    + " ON s.CodeId = x.CodeId"
                    + " SET"
                    + " t.code_type = IF(s.EmisCodeCategoryDescription != '', EmisCodeCategoryDescription, null),"
                    + " t.read_term = IF(s.Term != '', s.Term, null),"
                    + " t.read_code = IF(s.ReadTermId != '', s.ReadTermId, null),"
                    + " t.snomed_concept_id = IF(s.SnomedCTConceptId != '', s.SnomedCTConceptId, null),"
                    + " t.snomed_description_id = IF(s.SnomedCTDescriptionId != '', s.SnomedCTDescriptionId, null),"
                    + " t.snomed_term = IF(x.SnomedTerm != '', x.SnomedTerm, null),"
                    + " t.national_code = IF(s.NationalCode != '', s.NationalCode, null),"
                    + " t.national_code_category = IF(s.NationalCodeCategory != '', s.NationalCodeCategory, null),"
                    + " t.national_code_description = IF(s.NationalDescription != '', s.NationalDescription, null),"
                    + " t.parent_code_id = IF(s.ParentCodeId != '', s.ParentCodeId, null),"
                    + " t.adjusted_code = IF(x.AdjustedCode != '', x.AdjustedCode, null),"
