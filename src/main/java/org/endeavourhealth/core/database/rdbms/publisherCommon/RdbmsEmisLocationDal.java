    ...
    public void updateLocationStagingTable(String filePath, Date dataDate, int publishedFileId) throws Exception {
        ...
        try {
            ...
            sql = "INSERT IGNORE INTO emis_location (location_guid, location_name, location_type_description,"
                    ...
                    + " IF(LocationTypeDescription != '', LocationTypeDescription, null),"
                    + " IF(ParentLocationGuid != '', ParentLocationGuid, null),"
                    ...
                    + " IF(MainContactName != '', MainContactName, null),"
                    + " IF(FaxNumber != '', FaxNumber, null),"
                    + " IF(EmailAddress != '', EmailAddress, null),"
                    + " IF(PhoneNumber != '', PhoneNumber, null),"
                    + " IF(HouseNameFlatNumber != '', HouseNameFlatNumber, null),"
                    + " IF(NumberAndStreet != '', NumberAndStreet, null),"
            ...
            sql = "UPDATE emis_location t"
                    ...
                    + " SET"
                    + " t.location_name = IF(s.LocationName != '', s.LocationName, null),"
                    + " t.location_type_description = IF(s.LocationTypeDescription != '', s.LocationTypeDescription, null),"
                    + " t.parent_location_guid = IF(s.ParentLocationGuid != '', s.ParentLocationGuid, null),"
                    ...
                    + " t.main_contact_name = IF(s.MainContactName != '', s.MainContactName, null),"
                    + " t.fax_number = IF(s.FaxNumber != '', s.FaxNumber, null),"
                    + " t.email_address = IF(s.EmailAddress != '', s.EmailAddress, null),"
                    + " t.phone_number = IF(s.PhoneNumber != '', s.PhoneNumber, null),"
                    + " t.house_name_flat_number = IF(s.HouseNameFlatNumber != '', s.HouseNameFlatNumber, null),"
                    + " t.number_and_street = IF(s.NumberAndStreet != '', s.NumberAndStreet, null),"
                    + " t.village = IF(s.Village != '', s.Village, null),"
                    + " t.town = IF(s.Town != '', s.Town, null),"
                    + " t.county = IF(s.County != '', s.County, null),"
                    + " t.postcode = IF(s.Postcode != '', s.Postcode, null),"
