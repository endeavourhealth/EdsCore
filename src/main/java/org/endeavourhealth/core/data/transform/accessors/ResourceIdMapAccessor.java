...
public interface ResourceIdMapAccessor {
    ...
    @Query("SELECT * FROM subscriber.resource_id_map WHERE service_id = :service_id AND system_id = :system_id AND resource_type = :resource_type AND source_id = :source_id LIMIT 1")
    ...
    @Query("SELECT * FROM subscriber.resource_id_map_by_eds_id WHERE resource_type = :resource_type AND eds_id = :eds_id LIMIT 1")
